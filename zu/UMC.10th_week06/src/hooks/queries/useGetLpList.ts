...
function useGetLpList({cursor, search, order, limit}:PaginationDto) {
    return useQuery({
        ...
        queryFn: () =>
            ...
            // 데이터가 신선하다고 간주하는 시간,
            // 이 시간 동안은 캐시된 데이터를 그대로 사용합니다. 컴포넌트가 마운트 되거나 창에 포커스 들어오는 경우로 재요청X
            // 5분 동안 기존 데이터를 그대로 활용해서 네트워크 요청을 줄인다.
            staleTime: 1000*60*5,//5분

            // 사용되지 앟는 (비활성 상태) 인 쿼리 데이터가 캐시에 남아있는 시간.
            ...
            //refetchInterval:100*60, 10초마다 자동새로고침? 주식같은 거
            //retry:3, 기본값은 3회정도, 네트워크 오류 등 임시적인 문제를 보완할 수 있다.
    });
}
